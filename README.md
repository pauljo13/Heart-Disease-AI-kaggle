...
## 프로젝트 설명
이 프로젝트는 다양한 심장 질환 데이터 세트를 결합하여 만든 큰 데이터 세트를 활용해 심장 질환 예측 모델을 개발하고자 했습니다. 사용된 데이터는 5개의 다른 심장 질환 데이터 세트로부터 1,190개의 인스턴스와 11개의 공통 특성으로 구성되어 있습니다.
  
- 주요 단계 및 방법:
    - 데이터 준비 및 탐색:
        - 중복 값과 결측치 검사를 통해 데이터의 질을 향상시켰습니다.
        - 이상치를 식별하고 처리하여 데이터의 신뢰성을 높였습니다.
    - 모델 개발 및 학습:
        - 여러 머신러닝 모델을 통해 데이터를 학습시켰고, 그 중 XGBClassifier가 가장 높은 성능을 보였습니다.
        - 데이터의 이상치 처리 방법에 따라 다른 데이터 프레임에서도 성능을 비교 분석하였습니다.
    - 딥러닝 모델 학습:
        - PyTorch를 사용하여 딥러닝 모델을 실험했지만, 머신러닝 모델에 비해 낮은 성능을 보여 최종적으로 사용하지 않았습니다.
- 결과 평가 및 조정:
    - 최종 모델은 XGBClassifier로, 다양한 파라미터 조정과 피처 중요도 분석을 통해 모델의 성능을 극대화하였습니다.
  
결론:
이 프로젝트는 심장 질환 예측을 위한 강력한 머신러닝 모델을 개발하며, 데이터 처리와 모델 최적화 방법에 대한 심도 있는 이해를 제공하였습니다. XGBClassifier 모델이 평균값으로 대체된 데이터셋에서 높은 성능을 보였으며, 모델은 최대 90%의 정확도를 달성하였습니다. 이 프로젝트는 향후 심장 질환의 조기 진단과 치료에 기여할 수 있는 중요한 인사이트를 제공합니다.
...
- 모델 파라미터 조정
    - 학습한 모델의 파라미터의 수치를 변화시키거나 파라미터 값을 추가하여 새로운 모델을 학습하여 좀 더 상향되는 파라미터 수치와 파라미터를 찾는 과정을 수행한다.
- featur importances
    - 파라미터 조정이 끝난 모델의 컬럼별 영향을 살펴 필요없는 컬럼이 있는지 더 중요한 컬럼을 확인한다. 이를 통해서 데이터의 컬럼을 지우거나 하여 모델 학습도를 높인다.

## 결론
 모델은 최대학습도가 90 정도의 정확도를 보이는 모델을 제작하였다. 해당 모델에 약간의 부족한 점과 수정해야하는 부분 공부를 더 햐야하는 부분이 있다는 것을 알게 해준 프로젝트이다. 